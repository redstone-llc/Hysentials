...
function loadRanks() {
    ...
    axios.get(apiURL + "online", { headers: { "User-Agent": "Mozilla/5.0 (ChatTriggers)" }, parseBody: true }).then((onlineResponse) => {
        ...
        for (let user of onlineResponse.data.online) {
            if (!Scoreboard.getScoreboard().func_96509_i(target.getName()).func_96661_b().equals(teamMap.get(user))) {
                loadedUsers.delete(user)
            }
            if (!users.includes(user)) continue;
            if (loadedUsers.has(user)) continue;
            ...
            axios.get(apiURL + "rank", {
                ...
                let prefix = World.getPlayerByName(user).getDisplayName().getText().startsWith("§a◆ ") ? "" : "§a◆ "
                let char
                switch (rank) {
                    case "admin": {
                        prefix += "§c[ADMIN] "
                ...
                if (target != null) {
                    ...
                    teamMap.set(target.getName(), newTeam.func_96661_b())
                }
            })
